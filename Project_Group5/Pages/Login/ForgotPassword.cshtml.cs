...
namespace Project_Group5.Pages.Login
{
    public class ForgotPasswordModel : PageModel
    {
        ...
        private string GenerateToken(string email)
        {
            ...
            var key = Encoding.UTF8.GetBytes(_configuration["EmailSettings:SecretKey"]);
            using (var hmac = new HMACSHA256(key))
            {
                var tokenBytes = hmac.ComputeHash(Encoding.UTF8.GetBytes(data));
                var token = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{Convert.ToBase64String(tokenBytes)}:{timestamp}"));
                return token;
            }
        }
